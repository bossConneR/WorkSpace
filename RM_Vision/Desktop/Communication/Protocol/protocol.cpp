...
std::string Protocol::encode(const Vision_send_data& data)
{
    ...
    s.resize(tx_buf_len);
    for(int i = 0; i < s.size(); ++i) s[i] = ch_arr[i];
    return s;
}
...
*/
Protocol::Vision_recv_data Protocol::decode(const std::string& s)
{
    static char ch_arr[32];
    static uint16_t msg;
...
//    }
    ...
    return Vision_recv_data(static_cast<Self_color>(msg & 0xf),         // self_color
